...
class theme_features{
	...
	public static function get_old_theme_info($key = null){
		$theme = get_transient('theme_info') ? get_transient('theme_info') : false;
	...
	public static function set_theme_info(){
		...
		set_transient('theme_info',$theme);
	}
	/**
	 * get_theme_file_url
	 ...
	 */
	public static function get_theme_js($file_basename = null,$url_only = true,$mtime = false){
	 ...
	 */
	public static function get_theme_css($file_basename = null,$args = null,$mtime = false){
		$options = theme_options::get_options();
	...
	public static function get_theme_extension_url($args = null){
		...
		$options = theme_options::get_options();
		$dev_mode = isset($options['dev_mode']) && isset($options['dev_mode']['on']) ? true : false;
		$path_info = pathinfo($file_basename);
	 ...
	 * @author KM@INN STUDIO
	 */
	public static function get_theme_includes_js($FILE = null,$filename = 'init',$mtime = false){
	 ...
	 * @author KM@INN STUDIO
	 */
	public static function get_theme_includes_css($FILE = null,$filename = 'style',$mtime = false){
		$path_info = pathinfo($filename);
	...
	public static function get_theme_includes_image($FILE = null,$filename = null){
		...
		extract($r,EXTR_SKIP);
		return self::get_template_directory_uri() . '/' . $type . '/' . $basedir . $file_basename;
	}
	public static function get_theme_features_image($FILE = null,$filename = null){
		$args = array(
			'type' => 'features',
			'basedir' => basename(dirname($FILE)) . self::$basedir_images_min,
			'file_basename' => $filename,
	...
	/**
	 * theme_features::get_wp_title()
	 * 
	 * @return the wp title in <title> tags
	 * @version 1.0.2
	 * @author KM@INN STUDIO
	 * 
	 */
	public static function get_wp_title(){
		...
		return apply_filters('get_wp_title',$title);
	}
	/**
	 * get_link_page_url
	 *
	 ...
	 * @author KM@INN STUDIO
	 */
	public static function cat_option_list($tpl_cat_id,$child = false){
		$options = theme_options::get_options();
		if($child !== false){
			$tpl_cat_current_id = isset($options['tpl']['cat-id-' . $tpl_cat_id][$child]) && $options['tpl']['cat-id-' . $tpl_cat_id][$child] != 0 ? $options['tpl']['cat-id-' . $tpl_cat_id][$child] : null;
		}else{
			$tpl_cat_current_id = isset($options['tpl']['cat-id-' . $tpl_cat_id]) && $options['tpl']['cat-id-' . $tpl_cat_id] != 0 ? $options['tpl']['cat-id-' . $tpl_cat_id] : null;
		}
		$cat_args = array(
			'name' => $child !== false ? 'tpl[cat-id-' . $tpl_cat_id . '][' . $child . ']' : 'tpl[cat-id-' . $tpl_cat_id . ']',
			'id' => $child !== false ? 'tpl-cat-id-' . $tpl_cat_id . '-' . $child : 'tpl-cat-id-' . $tpl_cat_id,
			'show_option_none' => ___('Select category'),
			'hierarchical' => 1,
			'hide_empty' => false,
			'selected' => $tpl_cat_current_id,
			'echo' => 0,
		);		
	 ...
	 * 
	 *
	 * @param string $ids_name Option ID, $options['tpl']['cat-ids-' . $ids_name]
	 * @return string
	 * @version 1.0.1
	 * @author KM@INN STUDIO
	 */
	public static function cat_checkbox_list($ids_name){
		$options = theme_options::get_options();
		$cats = get_categories(array(
			'hide_empty' => false,
			'exclude' => 1
		));
		$cat_ids = (isset($options['tpl']) && isset($options['tpl']['cat-ids-' . $ids_name])) ? $options['tpl']['cat-ids-' . $ids_name] : array();
		...
		if($cats){
			...
			?>
			<label for="tpl-cat-id-<?php echo $ids_name;?>-<?php echo $cat->term_id;?>" class="tpl-item button <?php echo $selected_class;?>">
				<input 
					type="checkbox" 
					id="tpl-cat-id-<?php echo esc_attr($ids_name);?>-<?php echo esc_attr($cat->term_id);?>" 
					name="tpl[cat-ids-<?php echo esc_attr($ids_name);?>][]" 
	 ...
	 * @author KM@INN STUDIO
	 */
	public static function page_option_list($page_slug){
		$options = theme_options::get_options();
		$tpl_page_id = isset($options['tpl']) && isset($options['tpl']['page-id-' . $page_slug]) ? (int)$options['tpl']['page-id-' . $page_slug] : null;
		?>
		<select name="tpl[page-id-<?php echo esc_attr($page_slug);?>]" id="tpl-page-id-<?php echo esc_attr($page_slug);?>">
			<option value="-1"><?php echo esc_attr(___('Select page'));?></option>
			<?php
			$pages = get_pages();
			foreach($pages as $page){
				if($tpl_page_id == $page->ID){
					$selected = ' selected ';
				}else{
					$selected = null;
				}
