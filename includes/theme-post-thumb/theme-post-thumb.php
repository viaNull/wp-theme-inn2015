...
Version:		1.1.2
Description:	Agree or not? Just use the Thumb Up or Down to do it. <br/> 想要表达喜欢或讨厌？用日志顶踩功能来帮您实现
Author:			INN STUDIO
Author URI:		http://www.inn-studio.com
*/
theme_post_thumb::init();
class theme_post_thumb{
	private static $iden = 'post_thumb';
	public static function init(){
		...
		add_action('wp_ajax_post_thumb',get_class() . '::process');
		add_action('wp_ajax_nopriv_post_thumb',get_class() . '::process');
	}
			...
			<table class="form-table">
				<tbody>
					...
					<tr>
						...
						<td>
							<textarea id="post_thumb_up" name="<?php echo self::$iden;?>[text_up]" class="widefat" title="<?php echo ___('Thumb up');?>" cols="30" rows="10"><?php echo $post_thumb['text_up'];?></textarea>
						</td>
					</tr>
					<tr>
						<th scope="row"><label for="post_thumb_down"><?php echo ___('Thumb down');?></label></th>
						<td>
							<textarea id="post_thumb_down" name="<?php echo self::$iden;?>[text_down]" class="widefat" title="<?php echo ___('Thumb down');?>" cols="30" rows="10"><?php echo $post_thumb['text_down'];?></textarea>
						</td>
					</tr>
				</tbody>
			</table>
	...
	public static function process(){
		...
		if($post_id){
			...
			if(!self::is_voted($post_id)){
				...
				/* 写入曲奇 */
				$post_thumb = isset($_COOKIE['post_thumb']) ? (array)@json_decode(base64_decode($_COOKIE['post_thumb'])) : array();
				$post_thumb['ids'][] = $post_id;
				setcookie('post_thumb',base64_encode(json_encode($post_thumb)),time()+60*60*24*30*12,'/');
			}else{
				$output['status'] = 'error';
...
if(!function_exists('get_thumb_up')){
	function get_thumb_up(){
		return theme_post_thumb::get_thumb_up();
	}
}
if(!function_exists('get_thumb_down')){
	function get_thumb_down(){
		return theme_post_thumb::get_thumb_down();
	}
}
?>