	...
	public static function process(){
		...
		switch($type){
			...
			case 'upload':
				...
				if(!$file){
					$output['status'] = 'error';
					$output['id'] = 'invald_param';
					$output['msg'] = ___('Invalid param.');
					die(theme_features::json_format($output));
				}
				if($file['error'] != 0){
					$output['status'] = 'error';
					$output['id'] = 'file_error';
				...
				if(is_wp_error($attach_id)){
					$output['status'] = 'error';
					$output['id'] = $attach_id->get_error_codes();
			...
			case 'post':
				...
				if(empty($post_content)){
					$output['status'] = 'error';
					$output['id'] = 'invald_post_content';
					$output['msg'] = ___('Invalid post content.');
					die(theme_features::json_format($output));
				}
				/** post title */
				$post_title = isset($_POST['title']) && trim($_POST['title']) != '' ? htmlspecialchars($_POST['title']) : str_sub($post_content,20);
				...
				if(is_wp_error($post_id)){
					$output['status'] = 'error';
					$output['id'] = $post_id->get_error_code();
