...
class theme_comment_ajax{
	...
	private static function is_enabled(){
		...
		$is_enabled = isset($options['comment_ajax']) && isset($options['comment_ajax']['on']) ? true : false;
		return $is_enabled;
	}
	public static function backend_options_default($options){
		$options['comment_ajax']['on'] = 1;
		return $options;
	}
	public static function backend_options_save($options){
		$options['comment_ajax'] = isset($_POST['comment_ajax']) ? $_POST['comment_ajax'] : false;
		return $options;
	}
	public static function backend_options_display(){
		$options = theme_options::get_options();
		$is_checked = isset($options['comment_ajax']['on']) ? ' checked="checked" ' : null;
		?>
		<fieldset>
		...
		if(isset($_POST['comment_post_ID'])){
			 ...
			 */
				...
				if(get_option('comment_registration')|| 'private' == $status){
					$output['status'] = 'error';
					$output['des']['content'] = ___('Sorry, you must be logged in to post a comment.');
			...
			if(get_option('require_name_email')&& !$user->exists()){
				if(6 > strlen($comment_author_email)|| '' == $comment_author){
					$output['status'] = 'error';
					$output['des']['content'] = ___('Error: please fill the required fields(name, email).');
					die(theme_features::json_format($output));
				}else if(!is_email($comment_author_email)){
					$output['status'] = 'error';
					$output['des']['content'] = ___('Error: please enter a valid email address.');
					die(theme_features::json_format($output));
				}
			}
			/**
			...
			if(empty($comment_content)){
				$output['status'] = 'error';
				$output['des']['content'] = ___('Error: please type a comment.');
				die(theme_features::json_format($output));
			}
			/**
...
$comment_id = wp_new_comment($commentdata);
			...
			if($output['status'] === 'success'){
				...
				$output['des']['content'] = ___('Commented successfully, thank you!');
		 ...
		 */
			...
			if(!$cpage || !$post_id){
				$output['status'] = 'error';
				$output['des']['content'] = ___('Param error');
			}else{
				global $post;
				...
				$output['status'] = 'success';
				$output['des']['content'] = ___('Data sent.');
